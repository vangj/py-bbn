...
class Table(object):
    ...
    def __init__(self, node, parents=[]):
        ...
        if len(parents) == 0:
            ...
            cartesian = list(
                itertools.product(*[node.variable.values for node in self.parents])
            )
            get_kv = lambda i, v: f"{i}={v}"
            keys = [
                ",".join(
                    [get_kv(node.id, val) for node, val in zip(self.parents, values)]
                )
...
class LogicSampler(object):
    ...
    def __get_tables__(self):
        ...
        tables = [
            (node, [self.bbn.get_node(pa_id) for pa_id in parents])
            for node, parents in tables
        ]
    ...
    def __topological_sort__(self):
        ...
        """
        nodes = [
            SortableNode(node.id, set(self.bbn.get_parents_ordered(node.id)))
            for node in self.bbn.get_nodes()
        ]
        id_sorter = lambda x, y: -1 if x < y else 1 if x > y else 0
        pa_sorter = lambda x, y: (
            -1
            if x.node_id in y.parent_ids
            else 1 if y.node_id in x.parent_ids else id_sorter(x.node_id, y.node_id)
        )
